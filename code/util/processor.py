...
class Persent_Processor(DataProcessor):
    """Processor for the Sentihood data set."""

    def get_train_examples(self, data_dir):
        """See base class."""
        train_data = pd.read_csv(os.path.join(data_dir, "train/train_longformer_7topics.csv"))[['MASKED_DOCUMENT', 'sentiment', 'context']]
        return self._create_examples(train_data.values, "train")

    def get_dev_examples(self, data_dir):
        """See base class."""
        dev_data = pd.read_csv(os.path.join(data_dir, "dev/dev_longformer_7topics.csv"))[['MASKED_DOCUMENT', 'sentiment', 'context']]
        return self._create_examples(dev_data.values, "dev")

    def get_test_examples(self, data_dir):
        """See base class."""
        test_data = pd.read_csv(os.path.join(data_dir, "random_test/random_test_longformer_7topics.csv"))[['MASKED_DOCUMENT', 'sentiment', 'context']]
