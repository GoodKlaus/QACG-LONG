...
def getModelOptimizerTokenizer(model_type, vocab_file,
                               config_file=None, init_checkpoint=None,
                               label_list=None,
                               do_lower_case=True,
    ...
    if config_file is not None:
        model_config = BertConfig.from_json_file(config_file)
    else:
        # default?
    ...
    if init_checkpoint is not None:
        ...
        if "checkpoint" in init_checkpoint:
            ...
            if model_type == "QACGLONG":
                ...
                embedding_position_ids = model.bert.state_dict()["embeddings.position_ids"]

                state_dict = torch.load(init_checkpoint, map_location='cpu')
                from collections import OrderedDict
                new_state_dict = OrderedDict([(k.replace('longformer.',''), v) for k, v in state_dict.items()])
                remove_keys =  ["lm_head.bias", 
                                "lm_head.dense.weight", 
                                "lm_head.dense.bias", 
                                "lm_head.layer_norm.weight", 
                                "lm_head.layer_norm.bias", 
                                "lm_head.decoder.weight"]
                new_state_dict = OrderedDict([(k, v) for k, v in new_state_dict.items() if k not in remove_keys])
                ...
                for k, v in new_state_dict.items():
                    if k.endswith('LayerNorm.weight'):
                        ...
                        name = k
                    result_state_dict[name]=v

                result_state_dict.update({"embeddings.position_ids":embedding_position_ids})
                result_state_dict.move_to_end('embeddings.position_ids', last=False)
                
                model.bert.load_state_dict(result_state_dict, strict=False)
                # Update config to finetune token type embeddings
                model.config.type_vocab_size = 2 
...
def step_train(train_dataloader, test_dataloader, model, optimizer, 
    ...
    for step, batch in enumerate(pbar):
        ...
        if global_step != 0 and global_step % evaluate_interval == 0:
