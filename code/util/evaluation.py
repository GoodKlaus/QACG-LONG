...
def persent_strict_acc(y_true, y_pred):
    """
    Calculate "strict Acc" of aspect detection task of PerSent.
    """
...
def persent_strict_acc4(y_true, y_pred):
    """
    Calculate "strict Acc" of aspect detection task of PerSent.
    """
    total_cases=int(len(y_true)/4)
...
def persent_macro_F1(y_true, y_pred):
    """
    Calculate "Macro-F1" of aspect detection task of PerSent.
    """
    p_all=0
    r_all=0
    count=0
...
def persent_macro_F14(y_true, y_pred):
    """
    Calculate "Macro-F1" of aspect detection task of PerSent.
    """
    p_all=0
...
def persent_AUC_Acc(y_true, score):
    """
    Calculate "Macro-AUC" of both aspect detection and sentiment classification tasks of PerSent.
    Calculate "Acc" of sentiment classification task of PerSent.
    """
    # aspect-Macro-AUC
    aspect_y_true=[]
    aspect_y_score=[]
...
def persent_AUC_Acc4(y_true, score):
    """
    Calculate "Macro-AUC" of both aspect detection and sentiment classification tasks of PerSent.
    Calculate "Acc" of sentiment classification task of PerSent.
    """
...
def persent_PRF(y_true, y_pred):
    """
    Calculate "Micro P R F" of aspect detection task of PerSent.
    """
    s_all=0
    g_all=0
    s_g_all=0
...
def persent_Acc(y_true, y_pred, score, classes=4):
    """
    Calculate "Acc" of sentiment classification task of PerSent.
    """
    assert classes == 4, "classes must be 2 or 3 or 4."
    ...
    for i in range(len(y_true)):
        ...
        tmp=y_pred[i]
        if tmp==0:
            if score[i][1]>=score[i][2] and score[i][1]>=score[i][3]:
                tmp=1
            elif score[i][2]>=score[i][1] and score[i][2]>=score[i][3]:
                tmp=2
            elif score[i][3]>=score[i][1] and score[i][3]>=score[i][2]:
                tmp=3
